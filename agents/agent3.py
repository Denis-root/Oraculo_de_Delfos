"""
Tiene memoria checkpoint
+ memorias
+ el ultimo nodo responde en base a la info de todos los demas.
"""
...
from wrappers.MySqliteStore import SQLiteNamespaceVectorStore


#=============================Esta memoria es para los checkpoints
...
#====================Esta memoria es para las memories====================
MODEL_PATH = Path(__file__).resolve().parent.parent / "models" / "sentence-transformers" / "all-MiniLM-L6-v2"
ic(MODEL_PATH)
embedding_fn = HuggingFaceEmbeddings(model_name=str(MODEL_PATH))

# Crear el wrapper
BASE_DIR = Path(__file__).resolve().parent.parent
DB_PATH = BASE_DIR / "checkpoints" / "my_memories.sqlite"
DB_PATH.parent.mkdir(parents=True, exist_ok=True)
store = SQLiteNamespaceVectorStore(
    db_path=str(DB_PATH),
    embedding_function=embedding_fn,
    table="memories_delfos"
)
#====================Esta memoria es para las memories====================
...
def fn_save_memories(namespace: str, text, pregunta_usuario):
    ...
    for i, chunk in enumerate(chunks):
        ic(f"[Chunk {i + 1}]\n{chunk}\n---")
        store.add(namespace=namespace, texts=[chunk], metadatas=[{'pregunta': pregunta_usuario}])
        ic("memoria almacenada: ", namespace)


def fn_search_memories(namespace: str, query: str):
    resultados = store.search(namespace=namespace, query=query, k=5)
    memorias = "\n\n".join([
...
def sun_tzu(state: State):
    ...
    memories = None
    if "num_query" in state:
        memories = fn_search_memories(str(inspect.currentframe().f_code.co_name), "mensaje_actual_reescrito")
    ...
    *"Escucha la advertencia del Capítulo II: 'Nunca beneficia a un país prolongar la guerra'. Tu error es el mismo que asola a generales impacientes: confundir duración con fortaleza. Observa:*  
    ...
    if memories:
        human_prompt = f"""
        Estos son unos fragmentos historicos recolectados por medio de RAG, por si te sirven como contexto para responder a la consulta
        del usuario: 
        {memories}
        """

    human_prompt = human_prompt + f"""
    Consulta del usuario: {state['mensaje_actual_reescrito']}
    """
    ...
    fn_save_memories(str(inspect.currentframe().f_code.co_name), r.content, state['mensaje_actual_reescrito'])
...
def relator_del_consejo(state: State):
    ic(f"➡️FX: {inspect.currentframe().f_code.co_name}")
    system_prompt = """
    Actúa como un experto que responde preguntas con profundidad y claridad.  
    Recibirás la pregunta del usuario y una colección de textos previos (intervenciones de sabios, fuentes, opiniones, etc.).  
    Tu tarea es responder a la pregunta del usuario de manera **completa, explicativa y bien argumentada**, **utilizando toda la información relevante de los textos recibidos**.  
    No resumas, no relates, no hagas síntesis, no busques consensos.  
    Responde como un experto que domina el tema y tiene acceso a todas las fuentes proporcionadas.  
    Explica cada punto importante, usa ejemplos claros si es posible, y asegúrate de que la respuesta sea útil, concreta y detallada para el usuario.
    ...
    \"\"\"{state['mensaje_actual_reescrito']}\"\"\"
    ...
    cont = 0
    if "num_query" in state:
        cont = state['num_query'] + 1

    return {'messages': AIMessage(content=r.content), 'num_query': cont}


workflow = StateGraph(State)

workflow.add_node(nodo_parafraseo)
...
workflow.add_edge(START, "nodo_parafraseo")
workflow.add_edge("nodo_parafraseo", "sun_tzu")
